...
| `.cargo/config.toml` — `x86_64-unknown-none`, the linker script, `-C code-model=small -C relocation-model=static` | **Verified in CI.** All four settings appear on rustc's command line in `image-build`'s `--verbose` log, which also confirms the mandatory `cd` works: cargo found the file by walking up from the crate directory. The two `-C` flags override target defaults that assume a higher-half relocatable kernel. Not optional: `ld -shared` on this image fails with `R_X86_64_32 against __bss_start ... recompile with -fPIC`, so a PIC link is not something the image tolerates. |
| `adrian-kernel` built for `x86_64-unknown-none` | **Verified in CI, and this is new.** rustc 1.98.0, clean, `-C panic=abort -C opt-level=3`. The `no_std` configuration had only ever been built for the *host* with `--no-default-features` before this; a bare-metal target is a different question and it is now answered. The same log shows `--cfg 'feature="default"'` and no `std`, which is the first actual observation that the workspace exclusion does what it was built to do. |
| `src/main.rs` — `rian_main`, handoff report, call to `kernel_entry_and_halt` | **Does not compile yet.** `image-build` on `cd8d57b0` failed with `invalid asm template string: expected closing brace` at `main.rs:27`, because `boot.s` held a curly brace inside the comment stating that it must not hold one. Fixed; a grep guard now runs before the build on both remotes, since rustc blames the `include_str!` call site and never the offending line. The failure was at macro expansion, so everything downstream is still open: the integrated assembler has not seen `boot.s`, `rust-lld` has not used `rian.ld`, and whether `compiler_builtins` supplies the `memcpy`/`memset` the kernel needs is unknown. |
...
| CI — `image-build`, `image-boot` (GitLab); combined `image` job (GitHub) | **`image-build` has run once and failed at exit 101** (`cd8d57b0`), on the brace above; `image-boot` did not run because it `needs: image-build`. GitLab is split in two so a failure names which half broke; GitHub is one job because splitting there means artifact upload/download machinery on a remote that cannot run at all. Both now grep `boot.s` for braces before building. |
